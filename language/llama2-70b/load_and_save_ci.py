...
def get_args():
    ...
    parser.add_argument(
        "--quant_param_out_path", help="mlperf submisssion quantization parameters for calibraed layers"
    )
    parser.add_argument(
        "--quant_format_out_path", help="mulerf submission quantization specifications for calibrated layers"
    )
...
def test_model_equivalence():
    ...
    # create mlperf model for save qformat, qparam
    ...
    prefill_model = model_compressor.create_quantsim_model(
        ...
        act_zp_equalizing=(qconfig["act_zp_equalizing"] if qconfig["act_zp_equalizing"] else 'disabled'),
    ...
    model_compressor.save(
        prefill_model,
        qformat_out_path=args.quant_format_out_path,
        qparam_out_path=args.quant_param_out_path,
    ...
    args.model_source = "mlperf_submission"
    model = load_pytorch_model(args.model_source, args.model_path, args.n_layers)
    args.quant_format_path = args.quant_format_out_path
    args.quant_param_path = args.quant_param_out_path
    
    # create quant golden model and activate dump mode
    mlperf_path = "./mlperf_dump"
    mlperf_model = get_quant_model(model, args)
    model_compressor.set_model_to_dump_golden_model(
